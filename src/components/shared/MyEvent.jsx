...
const MyEvent = () => {
  ...
  const getCategory = (event) => {
    if (
      event.title.toLowerCase().includes("ux") ||
      event.title.toLowerCase().includes("design") ||
      event.title.toLowerCase().includes("figma")
    ) {
      return "design";
    }
    if (
      ...
      event.title.toLowerCase().includes("design ops")
    ) {
      return "tech";
    }
    return "general";
      ...
      {loading ? (
        ...
        <div className="grid gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {events.slice(0, showAll ? events.length : 11).map((event) => (
            ...
            >
              <div
                className={`card-xs p-4 h-fu1l rounded-lg ${getCategoryStyle(
                  getCategory(event)
                )}`}
              >
                <h3 className="text-md font-mono text-gray-900 line-clamp-1">
                  <span className="">Event Serie: </span>
